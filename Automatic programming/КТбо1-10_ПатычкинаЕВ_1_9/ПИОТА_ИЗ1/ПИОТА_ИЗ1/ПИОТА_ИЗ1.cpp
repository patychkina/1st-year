...
int main()
{
	...
	do
	{
		...
		else
		{
			...
			{
				mymachine.state = СharacterProcessing(mymachine.table, line[i], mymachine.state);
			}
...
//         выводит текущий символ и соответствующий ему переход состояния
//Выходные параметры: int t[state][num] - новое текущее состояние
int СharacterProcessing(int t[row][column], char c, int state)
{
	int num;
	if (c == '0')
